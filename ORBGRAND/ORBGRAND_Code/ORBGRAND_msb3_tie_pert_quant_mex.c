...
void bitonic_sort(double *arr, uint32_t *ind_order, uint64_t n) {
    ...
    /*
     * MSB3 tie-perturbation sorter
     *
     * Normal case:
     *   arr[i] != arr[l] -> standard MSB3 compare-swap
     *
     * Tie case inside selected region:
     *   instead of fixed keep or fixed swap,
     *   split the tied bucket:
     *
     *      min output = value - 1
     *      max output = value
     *
     ...
     *      001, 001 -> 000, 001
     *
     * This does NOT use LSBs.
     */

    const int DIST_THR = 128;   /* try 1, 2, 4, 8, 16 */
    ...
    for (w = 2; w <= n; w *= 2) {
        ...
        for (j = w / 2; j > 0; j /= 2) {
             ...
             */
            int perturb_region = in_last_stage && (j >= DIST_THR);
            ...
            for (i = 0; i < n; i++) {
                ...
                if (l > i) {
                    ...
                    if (arr[i] > arr[l]) {
                         ...
                         * Tie-perturbation:
                         *   do NOT swap indices.
                         *   But split the tied magnitudes at the CAE output.
                         */
                        if (perturb_region) {

                            double tied_val = arr[i];
                            double low_val;

                            if (tied_val > 0.0)
                                low_val = tied_val - 1.0;
                            else
                                low_val = 0.0;
