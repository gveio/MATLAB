 ...
 */
void bitonic_sort_msb3_tiebreak_last3(uint32_t *mag_q,
                                      uint32_t *ind_order,
                                      uint64_t n,
                                      const int B_MAG,
                                      const int MSB_NUM,
                                      const int LSB_NUM)
{
    ...
    for (uint64_t w = 2; w <= n; w <<= 1) {
        ...
        // last 3 stages => w >= n/4 (for n=256 -> w=64,128,256)
        int use_tiebreak = (w >= (n >> 2));
...
void ORBGRAND(double *y_decoded,double *n_guesses,double *y_soft,uint8_t *H,uint64_t n,uint64_t s,uint64_t n_guesses_max){
   ...
   bitonic_sort_msb3_tiebreak_last3(LLR_mag_q, sorted_list_q, n_effective,B_mag, MSB_NUM, LSB_NUM);
