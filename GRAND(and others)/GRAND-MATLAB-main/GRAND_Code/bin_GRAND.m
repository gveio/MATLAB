...
function [y_decoded,putative_noise,n_guesses,abandoned] = bin_GRAND(H,max_query,y_demod)
    ...
    n=size(H,2); %(n-k) x n -> H
    n_guesses = 0;%counter for noise pattern guesses
    abandoned = 0;%Flag indicating decoding status
    ...
    while n_guesses < max_query
        [decoded,putative_noise,ng]=bin_syn_check(H,y_demod,err_vec);%syndrome check
